...
export default function BalanceSheet() {
  ...
  return (
    <div className="card">
      <h2>{t('balanceSheetTitle')}</h2>
      <p style={{ fontSize: '0.8rem', color: '#6b6258' }}>{t('period')}: {label} ({from} to {to}) — {t('closingPositionShownAsOf')} {to}</p>
      <PeriodFilter vouchers={vouchers} openingDate={opening?.asOfDate} value={period} onChange={setPeriod} />

      <div className="grid-2">
        <div>
          <h3 style={{ fontSize: '0.9rem', color: 'var(--maroon)' }}>{t('fundsLiabilitiesByBranch')}</h3>
          <table>
            <thead><tr><th>{t('branch')}</th><th>{t('openingBF')}</th><th>{t('movement')}</th><th>{t('closing')}</th></tr></thead>
            <tbody>
            ...
            <tfoot>
              <tr style={{ fontWeight: 700 }}>
                <td>{t('totalFundsConsolidated')}</td>
        ...
        <div>
          <h3 style={{ fontSize: '0.9rem', color: 'var(--maroon)' }}>{t('assetsConsolidated')}</h3>
          <table>
            <thead><tr><th>{t('item')}</th><th>{t('openingBF')}</th><th>{t('movement')}</th><th>{t('closing')}</th></tr></thead>
            <tbody>
              <tr>
                <td>{t('fixedAssetsRow')}</td>
          ...
          <p style={{ fontSize: '0.7rem', color: '#6b6258', marginTop: 6 }}>
            {t('cashBankCentralNote')}
          </p>
        </div>
      </div>

      <div className="card" style={{ marginTop: 12 }}>
        <h3 style={{ fontSize: '0.9rem', color: 'var(--maroon)', marginTop: 0 }}>{t('fixedAssetsByBranch')}</h3>
        <table>
          <thead><tr><th>{t('branch')}</th><th>{t('closingValue')}</th></tr></thead>
          <tbody>
          ...
          <tfoot>
            <tr style={{ fontWeight: 700 }}><td>{t('totalFixedAssetsConsolidated')}</td><td>LKR {closing.fixedAssetsTotal.toLocaleString()}</td></tr>
          </tfoot>
        </table>
      </div>
      ...
      <div className="card" style={{ marginTop: 12, background: Math.abs(diff) < 1 ? '#eef7ee' : '#fdf0ee' }}>
        <p style={{ fontSize: '0.85rem', margin: 0 }}>
          <strong>{t('differenceAtClosing')}: LKR {diff.toLocaleString()}</strong>
          <br />
          {Math.abs(diff) < 1
            ? t('balancesMatch')
            : t('balanceMismatchNote')}
        </p>
      </div>
    </div>
  )
