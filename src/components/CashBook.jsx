...
export default function CashBook() {
  ...
  }, [vouchers, openingCash, openingDate, from, to])
  ...
  const closingBalance = rows.length > 0 ? rows[rows.length - 1].balance : openingCash
  ...
  return (
    <div className="card">
      ...
      <div className="summary-grid">
        <div className="summary-box"><div className="value">LKR {openingCash.toLocaleString()}</div><div className="label">Opening Cash</div></div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td></td><td></td>
            <td style={{ fontWeight: 600 }}>{openingCash.toLocaleString()}</td>
            <td></td>
          </tr>
          {rows.map((v) => (
            <tr key={v.id}>
              <td>{v.date}</td>
              <td>{particulars(v)}{v.type === 'Contra' ? '' : ''}</td>
